...
class ChangePeriod(Function):
    ...
    def _next(self):
        ...
        range_ = np.ptp(samples)

        fourier_transform = np.fft.rfft(samples / (0.5 * range_) - 1) / (0.5 * self.__range)

        magnitude = np.abs(fourier_transform)[::-1]
        target_magnitude = 0.5 * self.input[input_index - 1] * abs(self.__change) / range_
            ...
            if target_magnitude > magnitude[0]:
                for i, m in enumerate(magnitude):
                    if m >= target_magnitude:
                        ...
                        self._values.append(1 / frequency)
                        break
            else:
